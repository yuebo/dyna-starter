...
public class CurrentDepartmentManagerOptionProvider implements OptionProvider,AppConstants {
    ...
    public List<Map<String, String>> option(ViewContext viewContext, Map item, Map parameter) {
        ...
        if (request.getSession().getAttribute("user") != null) {
            ...
            String id = (String) user.get(DB_FIELD__ID);
            Map result = jdbcService.find("tbl_user_department", new BasicDBObject("userId", id));
            if (result != null) {
                Map department = jdbcService.find("tbl_department", new BasicDBObject(DB_FIELD__ID, result.get("departmentId")));
                if (department != null) {
                    Map manager = jdbcService.find(TBL_USER, new BasicDBObject(DB_FIELD__ID, department.get("managerId")));
