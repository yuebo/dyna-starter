...
//        System.out.println(filter);
        ...
        temp.putAll(filter);
        List<Map<String, Object>> searchResult = jdbcService.queryForListWithFilter("select * from (select r._id,r.rolename,group_concat(c.name)as permission from "+TBL_ROLE+" r left join "+TBL_ROLE_PERMISSION+" p on r._id=p.roleId left join "+TBL_PERMISSION+" c on p.permission=c._id group by r._id) temp", temp, sort, limit, skip);
