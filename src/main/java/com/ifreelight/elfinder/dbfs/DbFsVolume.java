...
public class DbFsVolume extends LocalFsVolume {
    ...
    public void createFile(FsItem fsi) throws IOException {
        ...
        if(!exists(fsi)){
            ...
            param.put("parent",asFile(getParent(fsi)).get("_id"));
...
//            file.delete();
    ...
    public void deleteFile(FsItem fsi) throws IOException {
        ...
        //deleteRemote
        qiniuService.delete(MapUtils.getString(file,"_id"));
    }
        ...
        if(result!=null){
            ...
            for(Map<String,Object> r:result){
                ...
                if("file".equals(MapUtils.getString(r,"type"))){
                    ids.add(MapUtils.getString(r,"_id"));
                }
    ...
    public FsItem[] listChildren(FsItem fsi) {
        ...
        if(objectId!=null){
            query.append("parent",asFile(fsi).get("_id"));
        }else {
            query.append("parent","");
        }
    ...
    public InputStream openInputStream(FsItem fsi) throws IOException {
        ...
        java.net.URLConnection urlConnection= new URL(qiniuApiConfig.getBaseUrl()+MapUtils.getString(asFile(fsi),"_id")+"?"+System.currentTimeMillis()).openConnection();
    ...
    public OutputStream openOutputStream(FsItem fsi) throws IOException {
        ...
        return new TempFileOutputStream(temp, new Callable() {
            ...
            public Object call() throws Exception {
                Map<String,Object> file=asFile(fsi);
                String id=MapUtils.getString(file,"_id");
                qiniuService.upload(temp,id);
                jdbcService.update(DATA_DBFS,new BasicDBObject("_id",id),new BasicDBObject("size",temp.length()));
                return 0;
            }
        });
    }
    ...
    public void rename(FsItem src, FsItem dst) throws IOException {
        ...
        jdbcService.update(DATA_DBFS,new BasicDBObject("_id",srcFile.get("_id")),new BasicDBObject("path",dst.getObjectId()).append("name",filename(dst)));
        if(isFolder(dst)) {
            BasicDBObject query=new BasicDBObject();
            if(src.getObjectId()!=null){
                query.append("parent",srcFile.get("_id"));
            }else {
                query.append("parent","");
            }
            //update parent
            ...
            if(files!=null){
                for(Map<String,Object> file:files){
                    jdbcService.update(DATA_DBFS,new BasicDBObject("_id",file.get("_id")),new BasicDBObject("path", StringUtils.replaceOnce(MapUtils.getString(file,"path"),src.getObjectId(),dst.getObjectId())));
                }
            }
            //update child path
    ...
    public String getURL(FsItem f) {
        ...
        return qiniuApiConfig.getBaseUrl()+MapUtils.getString(asFile(f),"_id")+"?view";
    }
